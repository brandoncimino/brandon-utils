...
namespace BrandonUtils.Standalone.Randomization {
    public static class Brandom {
        ...
        public static T FromWeightedList<T>(IDictionary<T, double> weightedChoices) {
            var totalWeight  = weightedChoices.Values.Sum();
            var targetWeight = Gen.NextDouble() * totalWeight;
        ...
        /// <inheritdoc cref="FromWeightedList{T}(System.Collections.Generic.IDictionary{T,double})"/>
        public static T FromWeightedList<T>(IDictionary<T, int> weightedChoices) {
            var doubleDic = weightedChoices.ToDictionary(it => it.Key, it => (double) it.Value);
            return FromWeightedList(doubleDic);
        }

        /// <inheritdoc cref="FromWeightedList{T}(System.Collections.Generic.IDictionary{T,double})"/>
        public static T FromWeightedList<T>(IDictionary<T, float> weightedChoices) {
            var doubleDic = weightedChoices.ToDictionary(it => it.Key, it => (double) it.Value);
            return FromWeightedList(doubleDic);
        }

        /// <inheritdoc cref="FromWeightedList{T}(System.Collections.Generic.IDictionary{T,double})"/>
        public static T FromWeightedList<T>(IEnumerable<T> choices, Func<T, double> weightSelector) {
            var weightedChoices = choices.ToDictionary(
                it => it,
                weightSelector
            );

            return FromWeightedList(weightedChoices);
        }
